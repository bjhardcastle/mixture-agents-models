...
def plot_model(
    ...
    agents: List[Agent],
    data: GenericData,
    sessions_to_plot: Optional[List[int]] = None,
    ...
    for i, session_idx in enumerate(sessions_to_plot):
        ...
        axes[i, 0].scatter(trials, session_data.choices, c='blue', alpha=0.6, s=20, label='Choices')
        axes[i, 0].scatter(trials, session_data.rewards, c='red', alpha=0.6, s=20, label='Rewards')
        ...
        if 'state_probs' in predictions and len(predictions['state_probs']) > 0:
            ...
            for state in range(n_states):
                axes[i, 1].plot(trials, state_probs[:, state], 
                               label=f'State {state + 1}', linewidth=2)
        ...
        if 'state_probs' in predictions and len(predictions['state_probs']) > 0:
            ...
            for agent_idx in range(n_agents):
                axes[i, 2].plot(trials, agent_weights[:, agent_idx],
...
def plot_recovery(
    ...
    if len(recovery_results['true_beta']) > 0:
        ...
        recovered_beta = np.concatenate([b.flatten() for b in recovery_results['recovered_beta']])
        
        axes[0].scatter(true_beta, recovered_beta, alpha=0.6)
        axes[0].plot([true_beta.min(), true_beta.max()], 
                    [true_beta.min(), true_beta.max()], 'r--', alpha=0.8)
        axes[0].set_xlabel('True β')
        axes[0].set_ylabel('Recovered β')
        axes[0].set_title(f'β Recovery (r = {recovery_results["correlations"].get("beta", 0):.3f})')
    ...
    if len(recovery_results['true_pi']) > 0:
        ...
        axes[1].set_title(f'π Recovery (r = {recovery_results["correlations"].get("pi", 0):.3f})')
        axes[1].grid(True, alpha=0.3)
    
    # Plot A recovery
    if len(recovery_results['true_A']) > 0:
        true_A = np.concatenate([A.flatten() for A in recovery_results['true_A']])
        recovered_A = np.concatenate([A.flatten() for A in recovery_results['recovered_A']])
        ...
        axes[2].set_title(f'A Recovery (r = {recovery_results["correlations"].get("A", 0):.3f})')
...
def plot_dynamic_routing_results(
    results,
    save_path: Optional[str] = None
) -> plt.Figure:
    """
    ...
    if hasattr(data, 'contexts') and data.contexts is not None:
        ...
        axes[0, 0].fill_between(trials[vis_trials], 0, 1, alpha=0.1, color='purple', label='Visual')
        axes[0, 0].fill_between(trials[aud_trials], 0, 1, alpha=0.1, color='orange', label='Auditory')
    ...
    if 'state_probs' in predictions and len(predictions['state_probs']) > 0:
        ...
        for state in range(n_states):
            axes[0, 1].plot(trials, state_probs[:, state], 
                           color=colors[state], label=f'State {state + 1}', linewidth=2)
...
def plot_agent_weights_matrix(
    ...
    for i in range(model.beta.shape[0]):
        for j in range(model.beta.shape[1]):
            ax.text(j, i, f'{model.beta[i, j]:.2f}', 
