...
class GenericData:
    ...
    def __post_init__(self) -> None:
        ...
        logger.debug(
            f"Initializing GenericData with {self.n_trials} trials, {self.n_sessions} sessions"
        )

        if len(self.choices) != len(self.rewards):
            logger.error(
                f"Data length mismatch: choices={len(self.choices)}, rewards={len(self.rewards)}"
            )
            raise ValueError("Choices and rewards must have same length")

        if len(self.choices) != self.n_trials:
            logger.error(
                f"Trial count mismatch: expected={self.n_trials}, actual={len(self.choices)}"
            )
        ...
        logger.info(
            f"GenericData initialized: {self.n_trials} trials, {self.n_sessions} sessions, "
    ...
    def get_session_data(self, session_idx: int) -> GenericData:
        ...
        return GenericData(
            ...
            trial_types=(
                self.trial_types[mask] if self.trial_types is not None else None
            ),
            ...
            reaction_times=(
                self.reaction_times[mask] if self.reaction_times is not None else None
            ),
...
class TwoStepData(GenericData):
    ...
    choices_2: npt.NDArray[np.integer] = field(
        default_factory=lambda: np.array([], dtype=int)
    )
    rewards_2: npt.NDArray[np.floating] = field(
        default_factory=lambda: np.array([], dtype=float)
    )
    states_2: npt.NDArray[np.integer] = field(
        default_factory=lambda: np.array([], dtype=int)
    )
    transitions: npt.NDArray[np.integer] = field(
        default_factory=lambda: np.array([], dtype=int)
    )
    ...
    def __post_init__(self) -> None:
        ...
        super().__post_init__()
        logger.debug(
            f"Validating TwoStepData with {len(self.choices_2)} second-stage choices"
        )
        ...
        if len(self.choices_2) > 0:
            ...
            for arr_name, arr in [
                ("choices_2", self.choices_2),
                ("rewards_2", self.rewards_2),
                ("states_2", self.states_2),
                ("transitions", self.transitions),
            ]:
                if len(arr) != self.n_trials:
                    logger.error(
                        f"Second stage data length mismatch: {arr_name}={len(arr)}, expected={self.n_trials}"
                    )
...
class DynamicRoutingData(GenericData):
    ...
    trial_stim: npt.NDArray[np.integer] = field(
        default_factory=lambda: np.array([], dtype=int)
    )
    trial_block: npt.NDArray[np.integer] = field(
        default_factory=lambda: np.array([], dtype=int)
    )
    trial_opto_label: npt.NDArray[np.integer] = field(
        default_factory=lambda: np.array([], dtype=int)
    )
    auto_reward_scheduled: npt.NDArray[np.bool_] = field(
        default_factory=lambda: np.array([], dtype=bool)
    )
    rewarded_stim: npt.NDArray[np.integer] = field(
        default_factory=lambda: np.array([], dtype=int)
    )
    stim_start_times: npt.NDArray[np.floating] = field(
        default_factory=lambda: np.array([], dtype=float)
    )
    ...
    def __post_init__(self) -> None:
        ...
        for attr_name in [
            ...
            if len(arr) > 0 and len(arr) != self.n_trials:
                logger.error(
                    f"Dynamic routing data length mismatch: {attr_name}={len(arr)}, expected={self.n_trials}"
                )
                raise ValueError(f"{attr_name} must match trial count")
        logger.info(
            f"DynamicRoutingData validation completed for {self.n_trials} trials"
        )


def load_generic_data(file_path: str | Path, **kwargs) -> GenericData:
    """
...
def _load_mat_data(file_path: Path, **kwargs: Any) -> GenericData:
    ...
    return GenericData(
        ...
        reaction_times=(
            np.squeeze(reaction_times) if reaction_times is not None else None
        ),
        metadata={"source_file": str(file_path)},
    )
...
def load_twostep_data(
    file_path: str | Path, rat_id: int | None = None, **kwargs: Any
) -> TwoStepData:
    """
    ...
    if rat_id is not None:
        ...
        rat_data = (
            data[f"rat_{rat_id}"] if f"rat_{rat_id}" in data else data["data"][rat_id]
        )
    else:
        rat_data = data
